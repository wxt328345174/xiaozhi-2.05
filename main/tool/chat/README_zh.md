...
## 1. 解决的问题
`TextInvokeTool` 是一个纯客户端的 MCP 扩展，通过复用 `Application::WakeWordInvoke` + `Protocol::SendWakeWordDetected` 这一条既有链路，实现在不改服务器协议的前提下，从设备端主动“发文本 → 触发 LLM 语音播报”。和原生的唤醒词路径一致，服务器会以 `type=listen,state=detect,text=…` 的格式收到消息，因此可以直接返回 TTS 音频。
...
```
请调用 MCP 工具 `self.text_invoke`，参数 text 为需要播报的内容；如需分片可传 max_len，默认不分片。
```
...
2. 当前 retry 固定 1 次，超过后包括网络中断、服务器未返回 `state=tts` 等情况都会被丢弃并打印 `FAIL` 日志。  
