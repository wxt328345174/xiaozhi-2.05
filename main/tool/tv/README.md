...
## 1. 功能简介
- 进入“看电视”场景后，会立即触发一次“拍照 + 中文解说”，随后自动定期触发下一次。
...
## 3. 工作原理
- `tv.start` 成功后立即调用 `TextInvokeTool::Submit(固定文本)` 触发一次解说。
- 之后进入“串行一次性计时”流程：只有在本轮解说完成并回到 `listening` 时，才启动下一轮计时并等待触发。
...
- 自动定期触发会产生持续网络与摄像头资源消耗，需关注内存与网络稳定性。

## 7. 口播策略说明
- 不在用户可见口播中透露具体间隔，仅描述为“自动定期”。
- 若用户追问，再说明“会自动定期触发”，避免误导或暴露配置差异。
